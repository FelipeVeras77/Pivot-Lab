...
lateral alterna Automático → Claro → Escuro e guarda a escolha. O padrão vive em
`js/state.js` (`tema: App.store.get("tema", "escuro")`) — troque para `"auto"` se
preferir seguir o sistema.
